...
public interface GameDao {
  ...
  String COMPLETED_GAMED_QUERY =
      "SELECT DISTINCT gm.* "
          + " FROM Game "
          + " AS gm"
          + " INNER JOIN Guess "
          + " AS gs ON gs.guess_id = gm.game_id "
          + " WHERE  gs.correct = gm.code_length"
          + " ORDER BY gm.started DESC, gs.submitted DESC";

  String INCOMPLETE_GAMED_QUERY =
      "SELECT "
          + "gm.* "
          + " FROM Game "
          + " AS gm"
          + " LEFT JOIN Guess "
          + " AS gs ON gs.guess_id = gm.game_id AND gs.correct = gm.code_length "
          + " WHERE gs.guess_id IS NULL"
          + " ORDER BY gm.started ASC ";

  String INCOMPLETE_MATCH_GAME_QUERY =
      "SELECT "
          + "gm.* "
          + " FROM Game "
          + " AS gm"
